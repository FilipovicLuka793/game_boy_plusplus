...
uint16_t Cpu::read_reg(register_type rt) {
  ...
  case RT_AF:
    return (this->a << 8) | (this->f.get_reg() & 0x00FF);
  case RT_BC:
    return (this->b << 8) | (this->c & 0x00FF);
  case RT_DE:
    return (this->d << 8) | (this->e & 0x00FF);
  case RT_HL:
    return (this->h << 8) | (this->l & 0x00FF);
...
uint8_t Cpu::get_ie_register() { return this->bus.get_ie_reg(); }
